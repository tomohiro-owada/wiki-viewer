...
- **MCP サーバー内蔵** — Claude Code 等の AI ツールから Wiki を直接参照可能
- **リバースプロキシ対応** — `-base` オプションでサブパス運用可能
...
| `-base` | (なし) | ベースパス (例: `/docs`)。リバースプロキシ配下で運用する場合に指定 |
...
wiki-viewer は [Model Context Protocol (MCP)](https://modelcontextprotocol.io/) サーバーを内蔵しています。HTTP サーバーと同じプロセスで `/mcp` エンドポイントとして自動的に起動します。

### 提供ツール

| ツール | 説明 |
|--------|------|
| `pages` | Wiki 一覧・ページ一覧を取得 |
| `detail` | 指定ページの生 Markdown を取得 |
...
`-base` オプションを使うと、nginx 等のリバースプロキシ配下でサブパス運用できます:

```bash
wiki-viewer -dir ./wikis -port 8080 -base /wiki
```
...
```nginx
location /wiki/ {
    proxy_pass http://localhost:8080/wiki/;
}
```
